...
namespace Hotkeys
{
    public class HotkeyTools
    {
        ...
        public static Dictionary<string, Hotkey> LoadHotkeys(Dictionary<string, Hotkey> hotkeyList, List<string> hotkeyNames, MainWindow parent)
        {
        ...
        {
            Debug.WriteLine($"Try Registering hotkey {ghk.displayName} {ghk.key} {ghk.modifier}");
            if (ghk == null)
            {
                Debug.WriteLine($"Could not register unknown hotkey, ghk is null");
                return false;
            }
            if (ghk.Register())
            {
            ...
            else
            {
                ...
                {
                    //if (warning) 
                    Debug.WriteLine("Could not register hotkey named " + ghk.displayName + ", key " + ghk.key);
                }
                else
                {
                    //if (warning)
                    Debug.WriteLine("Could not register unknown hotkey");
                }
                return false;
            }
        }
        ...
        public static string[] RegisterHotkeys(Dictionary<string, Hotkey> hotkeyList, bool warning = false)
        {
            ...
            foreach (KeyValuePair<string, Hotkey> hk in hotkeyList)
            {
                if (hk.Value.Key != string.Empty)
                {
                    if (!RegisterHotKey(hk.Value.ghk, false)) //register the key, add a warning to the list if it fails
                    {
                        ...
                        Debug.WriteLine($"Failed registering {hk.Key}");
                    }
                }
            }
            ...
            if (warningKeys.Count > 0)
            {
                ...
                //if (warning) MessageBox  ... (warningText);
            }
